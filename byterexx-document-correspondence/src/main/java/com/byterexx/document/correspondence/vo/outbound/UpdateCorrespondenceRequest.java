...
@NoArgsConstructor
@Data
public class UpdateCorrespondenceRequest extends BaseRequest {

    @JsonIgnore
    @NotBlank(message = "Missing Mandatory Field: {id}")
    @Size(max = 64, message = "Invalid Field Length: {id}")
    @Pattern(regexp = "^[a-zA-Z0-9_\\-]*$", message = "Invalid Field Value: {id}")
    private String id;
    ...
    private Map<@Size(max = 64, message = "Invalid Field Length: {templateVariableName}")
        @Pattern(regexp = "^[a-zA-Z0-9_\\-]*$", message = "Invalid Field Value: {templateVariableName}") String,
                @Size(max = 64, message = "Invalid Field Length: {templateVariableValue}")
                @Pattern(regexp = "^[a-zA-Z0-9_\\-@\\.]*$", message = "Invalid Field Value: {templateVariableValue}") String> templateVariables;

    private Map<@Size(max = 256, message = "Invalid Field Length: {attachmentName}")
    @Pattern(regexp = "^[a-zA-Z0-9_\\-]*$", message = "Invalid Field Value: {attachmentName}") String,
